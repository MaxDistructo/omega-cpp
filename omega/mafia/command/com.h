    ...
    class MafiaJoin : public mdcore::Command
    {
        public:
            ...
            void execute(SleepyDiscord::DiscordClient *client, SleepyDiscord::Message event, std::vector<std::string> args) override
            {
                ...
                {
                    event.reply(client, "The game is currently not accepting new participants. Please wait for a new game to be opened.");
                }
            };
    };
        ...
        public:
            ...
            void execute(SleepyDiscord::DiscordClient *client, SleepyDiscord::Message event, std::vector<std::string> args) override
            {
                ...
                if(args.size() == 3){
                    ...
                    if(errorCheck(targets))
                    {
                        event.reply(client, "One of your targets was invalid so we could not submit your request.");
                        return;
                    }
                    else{
